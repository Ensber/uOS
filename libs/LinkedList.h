...
public:
	...
	*/
	...
	/*
		Get the referenced element on the list;
		Return Element if accessible,
		else, return false;
	*/
	virtual int find(T elem);
	...
	*/
	...
	inline T& operator[](size_t& i) { return this->at(i); }
...
template<typename T>
int LinkedList<T>::find(T elem){
	return -1;
}

template<>
int LinkedList<String>::find(String elem){
	for (int i=0; i<size(); i++) {
		ListNode<String> *tmp = getNode(i);
		if (tmp && tmp->data == elem) return i;
	}
	return -1;
}
